...
function translationResponseFormat(
  ...
  const schema = z.object({ translations: z.object(translations).strict() }).strict();
  return zodResponseFormat(schema, TRANSLATION_RESPONSE_FORMAT_NAME);
}
...
function decodeTranslationOutput(value: unknown, key: string): ParsedTranslationOutput {
  if (!isRecord(value)) {
    throw new Error(`OpenAI returned an invalid translation for key "${key}".`);
  }
...
function decodeTranslationPayload(parsed: unknown): readonly ParsedTranslationItem[] {
  if (!isRecord(parsed)) {
    throw new Error("OpenAI returned an invalid parsed translation payload.");
  }
    ...
    const candidateOutputs = isRecord(value.candidates)
      ? Object.entries(value.candidates)
          ...
          .map(([candidateKey, candidate]) =>
            decodeTranslationOutput(candidate, `${key}.${candidateKey}`),
          )
      : undefined;
    const output = candidateOutputs === undefined ? decodeTranslationOutput(value, key) : undefined;
    return {
      key,
...
function semanticAuditResponseFormat() {
  return zodResponseFormat(SemanticAuditBatchSchema, SEMANTIC_AUDIT_RESPONSE_FORMAT_NAME);
}

function singleRequirementSemanticAuditResponseFormat(requests: readonly SemanticAuditRequest[]) {
  ...
  return zodResponseFormat(
    z.object({ audits: z.object(audits).strict() }).strict(),
...
function removeProtectedNumericsFromPart(
  ...
  const numericAtoms = [
    ...new Set(
      numerics.flatMap(({ raw }) => [raw, ...localizedNumericAtomCandidates(raw)]),
    ),
...
function protectedAssemblyPartMaximumLengths(
  ...
  const numericMaximumByMarker = new Map(
    protectedText.numerics.map(({ marker, raw }) => [
      marker,
      Math.max(...localizedNumericAtomCandidates(raw).map((candidate) => Array.from(candidate).length)),
    ]),
  );
  const fixedSlotCharacters = protectedText.assemblySlots.reduce(
    (total, slot) =>
      total +
      (numericMaximumByMarker.get(slot.marker) ??
        Array.from(slot.replacement ?? slot.raw).length),
    0,
  );
...
export interface OpenAiSemanticAuditPromptArgs {
  auditId: string;
  locale: string;
  modelId: string;
...
export type OpenAiSemanticAuditPrompt = string | ((args: OpenAiSemanticAuditPromptArgs) => string);
...
export interface OpenAiSemanticAuditProviderOptions {
  adversarialPrompt?: OpenAiSemanticAuditPrompt;
  apiKey?: string;
  batchSize?: number;
  ...
  concurrentRequests?: number;
  forwardPrompt?: OpenAiSemanticAuditPrompt;
  maxCharsPerBatch?: number;
  maxRetries?: number;
  reasoningEffort?: "high" | "low" | "max" | "medium" | "none" | "xhigh";
  requestTimeoutMs?: number;
...
export interface OpenAiSystemPromptArgs {
...
export type OpenAiSystemPrompt = string | ((args: OpenAiSystemPromptArgs) => string);
...
function protectedCodeSourceLiterals(value: string): readonly string[] {
  ...
  return [
    ...new Set(patterns.flatMap((pattern) => Array.from(value.matchAll(pattern), ([match]) => match))),
  ].toSorted((left, right) => right.length - left.length || left.localeCompare(right));
}
...
function protectRequestText(
  ...
  const localizedSubstitutionRules = (effectiveContext?.constraints ?? []).flatMap(
    (constraint) =>
  ...
  const substitutionSpans: Array<
    ProtectedLocalizedSubstitution & { end: number; start: number }
  > = [];
 ...
 */
function orderRequestsForBatching(
...
async function runWithWallClockTimeout<TResult>(
  ...
  const deadline = new Promise<never>((_resolve, reject) => {
    timeout = setTimeout(() => {
      controller.abort();
      reject(new Error(`OpenAI translation request exceeded ${String(timeoutMs)}ms.`));
    }, timeoutMs);
  });
...
function buildSemanticAuditSystemPrompt(
  args: OpenAiSemanticAuditPromptArgs,
  customPrompt: OpenAiSemanticAuditPrompt | undefined,
): string {
...
function rawSemanticAuditItems(parsed: unknown): readonly unknown[] {
  if (!isRecord(parsed)) {
    throw new Error("OpenAI returned a malformed semantic audit payload.");
  }
  const keys = Object.keys(parsed);
  if (keys.length !== 1 || keys[0] !== "audits" || !Array.isArray(parsed.audits)) {
    throw new Error("OpenAI returned a malformed semantic audit envelope.");
  }
...
function validateSemanticAuditPayload(args: {
  ...
  for (const rawItem of rawItems) {
    ...
    if (!requestMap.has(rawKey)) {
      throw new Error(`OpenAI returned unknown semantic audit key "${rawKey}".`);
    }
  ...
  if (unkeyedItems > missingKeyCount) {
    throw new Error("OpenAI returned an extra unkeyed semantic audit item.");
  }
...
/**
 * Routing hint for OpenAI prompt caching. The full system prompt varies per
...
function translationCompletionOptions(args: {
  ...
  reasoningEffort: OpenAiTranslationProviderOptions["reasoningEffort"];
...
function buildSystemPrompt(
  ...
  customPrompt?: OpenAiSystemPrompt,
): string {
  const customPromptText =
...
function semanticAuditCompletionOptions(args: {
  ...
  return {
    ...
    response_format: semanticAuditResponseFormat(),
    ...(args.temperature === undefined ? {} : { temperature: args.temperature }),
  };
}

export interface OpenAiTranslationOutputContractMaterial {
...
export const OPENAI_TRANSLATION_OUTPUT_CONTRACT_MATERIAL: OpenAiTranslationOutputContractMaterial =
  {
    ...
    responseFormat: {
      standard: JSON.parse(JSON.stringify(translationResponseFormat())) as unknown,
      withSelfCheck: JSON.parse(
        JSON.stringify(
...
export function createOpenAiTranslationOutputContractRevision(
  material: unknown = OPENAI_TRANSLATION_OUTPUT_CONTRACT_MATERIAL,
): string {
  return `sha256:${createHash("sha256").update(JSON.stringify(material)).digest("hex")}`;
}

export const OPENAI_TRANSLATION_OUTPUT_CONTRACT_REVISION: string =
  createOpenAiTranslationOutputContractRevision();

export interface OpenAiSemanticAuditOutputContractMaterial {
  readonly implementation: {
...
export const OPENAI_SEMANTIC_AUDIT_OUTPUT_CONTRACT_MATERIAL: OpenAiSemanticAuditOutputContractMaterial =
  {
    implementation: {
      batchingAndSalvage: [
        ...
        singleRequirementSemanticAuditResponseFormat,
    ...
    responseFormat: JSON.parse(JSON.stringify(semanticAuditResponseFormat())) as unknown,
    schemaVersion: 2,
  };

export function createOpenAiSemanticAuditOutputContractRevision(
  material: unknown = OPENAI_SEMANTIC_AUDIT_OUTPUT_CONTRACT_MATERIAL,
): string {
  return `sha256:${createHash("sha256").update(JSON.stringify(material)).digest("hex")}`;
}

export const OPENAI_SEMANTIC_AUDIT_OUTPUT_CONTRACT_REVISION: string =
  ...
  return cleanParts
    .map((rawPart, index) => {
      ...
      const numericSource =
        protectedSlot === undefined
          ? undefined
          : numericSourceByMarker.get(protectedSlot.marker);
...
export class OpenAiTranslationProvider implements TranslationProvider {
  ...
  constructor(options: OpenAiTranslationProviderOptions = {}) {
    if (!options.client && !options.apiKey) {
      throw new Error("OpenAiTranslationProvider requires either apiKey or client.");
    }
  ...
  protected async translateBatch(args: {
    ...
    const completion = await this.requestLimiter.run(() =>
      runWithWallClockTimeout(this.requestTimeoutMs, (signal) =>
        this.client.chat.completions.parse(
          ...
          prompt_cache_key: translationPromptCacheKey({
            locale: args.locale,
            model: this.model,
            systemPrompt: this.systemPrompt,
          }),
          response_format: translationResponseFormat(
            args.batch.map((request) => ({
          ...
          { maxRetries: 0, signal, timeout: this.requestTimeoutMs },
        ),
      ),
    );
    ...
    if (!parsed) {
      throw new Error("OpenAI returned an empty parsed translation payload.");
    }
    const parsedTranslations = decodeTranslationPayload(parsed);
    ...
    if (
      ...
      throw new Error(
        `OpenAI returned an invalid translation key set.${
    ...
    for (const translation of parsedTranslations) {
      ...
      if (originalKey === undefined) {
        throw new Error(`OpenAI returned unknown translation key "${translation.key}".`);
      }
      const request = requestMap.get(originalKey);
      if (request === undefined) {
        throw new Error(`OpenAI returned unmapped translation key "${translation.key}".`);
      }
      if (
    ...
    if (
      ...
      const details = args.batch
        ...
        .join(", ");
      throw new Error(`OpenAI returned invalid one-shot translation output: ${details}.`);
    }
  ...
  private async translateBatchWithRetries(args: {
      ...
      for (const outcome of outcomes) {
        ...
        if ("error" in outcome) {
          ...
          const invalidError = new Error(
            `OpenAI omitted or returned an invalid translation for ${unresolved
    ...
    throw new Error(
      `OpenAI translation batch ${String(args.batchIndex + 1)} for locale ${args.locale} failed after ${String(attempts)} attempt(s) with ${String(args.batch.length - completed.size)} unresolved request(s) (${args.batch
...
export class OpenAiSemanticAuditProvider implements SemanticAuditProvider {
  private readonly adversarialPrompt: OpenAiSemanticAuditPrompt | undefined;
  private readonly batchSize: number;
  ...
  private readonly concurrentRequests: number;
  private readonly forwardPrompt: OpenAiSemanticAuditPrompt | undefined;
  private readonly maxCharsPerBatch: number;
  private readonly maxRetries: number;
  private readonly reasoningEffort:
    | "high"
  ...
  constructor(options: OpenAiSemanticAuditProviderOptions = {}) {
    if (!options.client && !options.apiKey) {
      throw new Error("OpenAiSemanticAuditProvider requires either apiKey or client.");
    }
  ...
  protected async auditBatch(args: {
    ...
    const promptArgs: OpenAiSemanticAuditPromptArgs = {
      auditId: args.auditId,
      locale: args.locale,
      modelId: args.modelId,
      pass: args.pass,
      ...
      this.client.chat.completions.parse(
        {
          ...
          response_format: this.singleRequirementRequests
            ? singleRequirementSemanticAuditResponseFormat(aliased.batch)
            : semanticAuditResponseFormat(),
        },
    ...
    if (parsed === null || parsed === undefined) {
      throw new Error("OpenAI returned an empty parsed semantic audit payload.");
    }
    const validated = validateSemanticAuditPayload({
      modelId: args.modelId,
  ...
  private async auditBatchWithRetries(args: {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt += 1) {
      ...
      for (const outcome of outcomes) {
        ...
        if (outcome.result !== undefined) {
          ...
          if (unresolved.length > 0) {
            lastError = new Error(
              `OpenAI omitted or returned an invalid semantic audit for ${unresolved
                .map(
    ...
    throw new Error(
      `OpenAI semantic audit ${args.pass} batch ${String(args.batchIndex + 1)} for locale ${args.locale} failed after ${String(attempts)} attempt(s); unresolved keys: ${pendingKeys.join(", ")}. ${errorMessage(lastError)}`,
      { cause: lastError },
    );
  }
}
